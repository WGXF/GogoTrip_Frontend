    ...
    <div className="flex items-center gap-4 p-4 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700/50 hover:border-sky-400 dark:hover:border-indigo-400 hover:shadow-2xl hover:shadow-sky-100 dark:hover:shadow-indigo-900/20 transition-all duration-300 ease-out group cursor-pointer hover:-translate-y-1 hover:scale-[1.02]">
      ...
      <div className={`flex flex-col items-center justify-center w-14 h-14 rounded-2xl ${dateBg} relative overflow-hidden flex-shrink-0 transition-transform group-hover:scale-110 duration-300`}>
        ...
        <span className={`text-[10px] font-bold uppercase tracking-widest ${dateText} mb-0.5`}>
          {appointment.date.toLocaleString('en-US', { month: 'short' })}
        </span>
...
const DashboardView: React.FC = () => {
  return (
    <div className="p-6 md:p-8 w-full mx-auto space-y-8 animate-fade-in-up">
      ...
      {/* 1. Travel Dashboard Header Container */}
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-10">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 font-medium text-lg">Your next adventure to Kyoto begins in 14 days.</p>
            </div>
            <div className="flex gap-3">
              ...
              <button className="px-5 py-3 bg-sky-600 dark:bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-sky-500 dark:hover:bg-indigo-500 transition-all shadow-lg shadow-sky-600/30 dark:shadow-indigo-600/30 flex items-center gap-2 active:scale-95 hover:-translate-y-1 hover:shadow-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <StatCard 
              ...
              value="3" 
              badgeText="NEXT: KYOTO (2D)"
              icon={<Plane className="w-5 h-5" />} 
            />
            <StatCard 
              title="Countries Visited" 
              value="12" 
              badgeText="+2 THIS YEAR" 
              badgeColor="green"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* 2. Upcoming Itinerary Container */}
        <div className="lg:col-span-2 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-[2.5rem] p-8 h-fit shadow-sm border border-transparent hover:border-sky-300 dark:hover:border-indigo-500 transition-all duration-500 ease-out hover:shadow-xl hover:bg-white/70 dark:hover:bg-slate-900/70">
          <div className="flex items-center justify-between mb-6">
            ...
            <button className="text-sm text-sky-600 hover:text-sky-500 dark:text-indigo-400 dark:hover:text-indigo-300 font-bold hover:underline">View Full Schedule</button>
          </div>
        ...
        <div className="space-y-8 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
          ...
          <div className="bg-gradient-to-b from-sky-500 to-blue-600 dark:from-indigo-600 dark:to-violet-700 rounded-[2.5rem] p-8 relative overflow-hidden shadow-xl shadow-sky-500/20 dark:shadow-indigo-500/20 group text-center border border-transparent hover:border-white/50 transition-all duration-300 hover:-translate-y-2 hover:scale-[1.02] hover:shadow-2xl">
            ...
            <div className="relative z-10">
              <button className="w-full py-3.5 bg-white text-sky-600 hover:bg-sky-50 dark:bg-white/10 dark:backdrop-blur-md dark:text-white dark:hover:bg-white/20 text-sm font-bold rounded-xl transition-all shadow-lg active:scale-95 border border-transparent hover:border-sky-100 hover:shadow-xl hover:-translate-y-1">
                Ask AI Planner
              </button>
            </div>
          </div>
          ...
          {/* 4. Saved Drafts Card */}
           <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-[2.5rem] p-8 shadow-sm border border-transparent hover:border-sky-300 dark:hover:border-indigo-500 transition-all duration-500 ease-out hover:shadow-xl hover:bg-white/70 dark:hover:bg-slate-900/70">
            ...
            <div className="space-y-3">
              <div className="flex gap-4 p-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-2xl transition-all cursor-pointer group border border-slate-100 dark:border-slate-700/50 hover:border-sky-400 dark:hover:border-indigo-400 hover:shadow-xl hover:-translate-y-1 hover:scale-[1.02] duration-300">
                   ...
                   <div>
                     <p className="text-sm text-slate-900 dark:text-white font-bold group-hover:text-sky-600 dark:group-hover:text-indigo-400 transition-colors">Weekend in Napa</p>
                     <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 font-medium">3 days • Wine Tasting</p>
                   </div>
                </div>
            </div>
          </div>
