...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, isOpen }) => {
  ...
  const menuItems = [
    { id: ViewState.DASHBOARD, label: 'Travel Dashboard', icon: LayoutDashboard },
    { id: ViewState.CHAT, label: 'AI Planner', icon: Sparkles },
    { id: ViewState.TRAVEL, label: 'My Trips', icon: Plane },
    { id: ViewState.SCHEDULER, label: 'Notes', icon: NotebookPen },
    { id: ViewState.TRANSLATE, label: 'Translate', icon: Languages },
    { id: ViewState.EXPENSES, label: 'Expenses', icon: Wallet },
    { id: ViewState.CALENDAR, label: 'Calendar', icon: CalendarDays },
  ];
           ...
           <div className={`mb-6 transition-all duration-500 delay-100 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
             ...
             `}>
               <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-2 space-y-1">
                  <button onClick={() => onChangeView(ViewState.SETTINGS)} className="w-full flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700 transition-colors active:scale-95">
                     <Settings className="w-4 h-4" />
                     Preferences
                  </button>
                  <button className="w-full flex items-center gap-3 px-3 py-3 rounded-lg text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-700 transition-colors active:scale-95">
           ...
           <nav className="space-y-2">
             {menuItems.map((item, index) => (
                ...
                className={`
                  ...
                  active:scale-105 active:font-semibold active:bg-slate-100 dark:active:bg-slate-800
                `}
              >
                <item.icon className={`w-6 h-6 ${currentView === item.id ? 'text-sky-600 dark:text-indigo-400' : 'text-slate-400'}`} />
          ...
          <div className="relative pt-6 px-4 mb-2" ref={logoMenuRef}>
             ...
             `}>
               <div className="p-1.5 space-y-0.5">
                  <button onClick={() => onChangeView(ViewState.SETTINGS)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                     <Settings className="w-4 h-4" />
                     Preferences
                  </button>
                  <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                     <HelpCircle className="w-4 h-4" />
                     Help & Support
                  </button>
                  <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  `}
                >
                  {/* Icon Container - Always centered when collapsed, left aligned when expanded */}
                  ...
                  <span className={`
                    ...
                    ${isActive ? 'text-sky-600 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-400'}
