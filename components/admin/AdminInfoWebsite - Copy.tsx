...
import { 
  ...
  Eye as ViewIcon, BarChart3, CheckCircle2, Clock, 
  MoreVertical, Save, AlertCircle, AlertTriangle, XCircle,
  Bold, Italic, Underline, Strikethrough, List, ListOrdered,
  AlignLeft, AlignCenter, AlignRight, AlignJustify,
  Quote, Code, Link as LinkIcon, Image as ImageIcon, Undo, Redo,
  Type, Heading1, Heading2, Heading3, Palette, Minus
} from 'lucide-react';
  ...
  return (
    <div className="flex flex-wrap items-center gap-0.5 p-2 border-b border-slate-200 bg-slate-50/80">
      ...
      <div className="relative">
        ...
        {showColorPicker && (
          <div className="absolute top-full left-0 mt-1 p-2 bg-white rounded-lg shadow-xl border border-slate-200 z-50 grid grid-cols-5 gap-1">
            {colors.map(color => (
              <button
                key={color}
                type="button"
            ...
            <button
              ...
              className="w-6 h-6 rounded border border-slate-200 text-xs text-slate-400 hover:bg-slate-100"
            >
              ✕
            </button>
          </div>
